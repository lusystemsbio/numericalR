...
contour_segment_bilinear <- function(x_all, y_all, z, ind_x, ind_y, npoints = 20){
  ...
  if(length(unique(sign(all_points))) == 1){ # all the same sign, no solution
    return()
      ...
      f_x_y1 = (x2 - x_points)*f11 + (x_points - x1)*f21  # equation (3)
      f_x_y2 = (x2 - x_points)*f12 + (x_points - x1)*f22  # equation (3)
      y_points = (f_x_y1*y2 - f_x_y2*y1)/(f_x_y1 - f_x_y2) # equation (4)
