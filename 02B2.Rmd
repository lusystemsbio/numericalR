...
``` {r}
plot(t, log10(nt1), xlab="t", ylab="log10(N)", xlim=c(0,100), ylim=c(0,4))
```
...
?deSolve # check help page if needed
dN_deSolve <- function(t, N, parms) { 
  return(list(r*N))
}
parms = c(r = 0.1)
N0 = 1
t_all_1 = seq(0, 100, 0.01)  # all t steps during the whole simulation, dt = 0.01
results_1 = ode(y = N0, times = t_all_1, func = dN_deSolve, parms = NULL, method = "euler")

t_all_2 = seq(0, 100, 0.1)  # all t steps during the whole simulation, dt = 0.1
results_2 = ode(y = N0, times = t_all_2, func = dN_deSolve, parms = NULL, method = "euler")

t_all_3 = seq(0, 100, 1)  # all t steps during the whole simulation, dt = 1
results_3 = ode(y = N0, times = t_all_3, func = dN_deSolve, parms = NULL, method = "euler")
