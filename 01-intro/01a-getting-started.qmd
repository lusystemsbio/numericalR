...
This book illustrates numerical methods in systems biology using R and Python side by side, with [Quarto](https://quarto.org) rendering both into one document and [VS Code](https://code.visualstudio.com) as the editor.

To set up your environment, install, in this order:

1. **R**, from [CRAN](https://cran.r-project.org).
2. **Python** (3.x), from [python.org](https://www.python.org/downloads/) or your system's package manager.
3. **Quarto**, from [quarto.org/docs/get-started](https://quarto.org/docs/get-started/).
4. **VS Code**, from [code.visualstudio.com](https://code.visualstudio.com), then add three extensions from the Extensions panel: **R** (by REditorSupport), **Python** (by Microsoft), and **Quarto**.
5. **Claude Code**, a command-line AI assistant that reads, edits, and runs code inside your own project rather than a generic chat window. Install it with

   ```bash
   npm install -g @anthropic-ai/claude-code
   ```

   which requires [Node.js](https://nodejs.org). Anthropic also publishes a VS Code extension for it, so a Claude Code panel can sit next to your editor; check Anthropic's current documentation for the exact setup, since install methods change over time.
...
1. **Understand the method before checking any code.** From the concept and the equations, decide what the result should be and how you will know it is correct, for instance you expect to observe a Hopf bifurcation for a repressilator gene regualtory circuit.
2. **Read the provided implementation** to see one correct version, and ask the AI assistant to explain any line you do not recognize or have doubts about.
3. **Rebuild the method yourself** after reading the provided implementation. You could focus on the parts related to thecore method, while letting the AI assistant fill in parts you are familiar with.
4. **Verify your results against the reference** once you have rebuilt the method. You could run your version and the book's code on the same input and compare the results, includign both the final and intermediate results, side by side. The verification step allows you to check your understanding of the method, and is also a good opportunity to ask the AI assistant to explain any discrepancy between your results and the reference.
5. **Generalize for other usages**: change the model, add a variable, alter a boundary. You could ask the AI assistant to help you with the generalization, but you should understand the changes you are making and how they affect the method. By this time, you should be able to make these changes without the AI assistant, and this is a good opportunity to test your understanding of the method and the code.
...
To run this yourself, create a new file `classify.R` (or `classify.py`) in VS Code, paste in the code above, and use the Run button the R or Python extension adds to the editor, or run `Rscript classify.R` / `python3 classify.py` from the integrated terminal.
...
Verify it: call `classify(11)` and confirm you get `"odd"`, then try `classify(0)` and check the answer still makes sense.
...
- **Predict, then run.** Before running, tell the assistant what you expect each line to print and ask whether you are right. Being wrong out loud is where the learning happens.
- **Translate and diff.** Ask it to rewrite the R version in Python (or the reverse), run both, and confirm the outputs match. A translation that agrees line for line means you understood the original.
- **Change one thing.** Ask what happens if you replace `== 0` with `== 1`, then check the prediction by editing and re-running.

Each of these keeps you doing the thinking and uses the assistant to check it, which is the habit the rest of the book is built around.
...
**Ellipsis and `**kwargs`.** R's `...` and Python's `**kwargs` let a function forward whatever arguments it received on to another function whose full signature it does not need to know. This pattern shows up in nearly every integrator in this book, starting with `Euler()` in [Part 2B](../02-odes/02b-numerical-integration.qmd).
...
Python draws the same line between a NumPy array (one type, for linear algebra) and a pandas data frame (a column per variable, mixed types).
...
Ask the assistant why a matrix cannot hold a text column alongside numbers, and when you would reach for each. Verify it: try to place a string into the numeric matrix (`matrix(c(1, "a", 3, 4), 2)` in R, `np.array([1, "a", 3, 4])` in Python) and watch every element turn into text, since a matrix or array must share one type.

## 1A.5 A worked example: Conway's Game of Life

Conway's Game of Life is a grid of cells, each alive or dead, updated together at every step by three rules applied to each cell's eight neighbors [@gardner1970game]:
...
::: {.content-visible when-format="html:js"}
![Forty generations of a glider on a 10x10 grid. Red cells are alive, white cells are dead.](images/01A/game-of-life.gif){width=40% fig-align="center"}
:::
...
Verify it: pick a cell in the top row and compute `((0 - 1 + -1) %% 10) + 1` by hand to confirm it does map to row 10, not to a negative or out-of-range index.

Periodic boundaries are not just a trick for a small grid of cells. [Part 5C](../05-molecular-dynamics/05c-box-of-particles.qmd) applies the same idea to a continuous-space molecular dynamics simulation: a particle leaving one side of a simulation box re-enters from the opposite side, using a function (`bc_periodic`) built on the same wraparound rule shown here.
