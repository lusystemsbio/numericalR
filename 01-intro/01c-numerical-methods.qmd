...
for a chosen tolerance $\epsilon$. (This is in fact a special case of Newton's method for finding the root of $f(x) = x^2 - a$; [Part 2F](../02-odes/02f-bifurcation-curves.qmd) implements Newton's method for a general $f$.)
...
The method converges fast: each iteration roughly doubles the number of correct digits. Tracking the digits of accuracy against $\sqrt{2}$ directly confirms this, and also shows machine precision (1C.2) in action:
