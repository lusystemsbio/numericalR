...
<br/>
Here, the promoter of the gene $X$ does not play a role in this process. The symbol $\varnothing$ means “null”, *i.e.*, a protein $X$ is destroyed in the degradation step. Now, we onsider a simplified model, in which the transcription and translation steps are described together by a single zero-order reaction with transcription rate $g$.
...
\begin{cases} \frac{dX}{dt} = g_{X0}\frac{1}{1+(Y/Y_0)^{n_Y}} - k_XX \\
              \frac{dY}{dt} = g_{Y0}\frac{1}{1+(X/X_0)^{n_X}} - k_YY \end{cases} \tag{13}
\end{equation}

In this case, the system is described by two coupled differential equations, the solutions to which are in the form of $X(t)$ and $Y(t)$. As there are two variables $X$ and $Y$, more sophisticated tools are required to understand this type of dynamical systems. We will discuss about it in both Part 2C and Part 3. 
