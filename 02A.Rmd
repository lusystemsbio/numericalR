...
\begin{equation}
\begin{cases} \frac{dX}{dt} = g_{Y0} + g_{X1}\frac{1}{1+(Y/Y_0)^{n_Y}} - k_XX \\
              \frac{dY}{dt} = g_{Y0} + g_{Y1}\frac{1}{1+(X/X_0)^{n_X}} - k_YY \end{cases} \tag{13}
\end{equation}

In this case, the system is described by two coupled differential equations, the solutions to which are in the form of $X(t)$ and $Y(t)$. As there are two variables $X$ and $Y$, more sophisticated tools are required to understand this type of dynamical systems. We will discuss about it in Part 3. 
