...
We generalize the previous PDE integrator for a two-component system. Here, we choose to use the periodic boundary condition by setting the state variables at the boundary points the same. 
...
par(mar = c(4, 4, 1, 1))
set.seed(10)
n = 2; len = 50; dX = 1; dt = 0.01; t.total_iter = 20
D = c(0.1,1); mu = 1.5
...
for(i in 0:5){
  ...
  plot(X_all, u_1[1,], type="l", col=1, xlab="X", ylab="u or v", xlim=c(0,50), ylim=c(0,2)) 
...
When $d = 0.5$ and $\mu = 0.8$, the system becomes of a traveling wave (partially due to the periodic boundary condition).

``` {r,fig.width = 4, fig.height = 3}
par(mar = c(4, 4, 1, 1))
D = c(0.7,1); mu = 0.3; t.total_iter = 20
set.seed(10)
...
for(i in 0:5){
  ...
  plot(X_all, u_1[1,], type="l", col=1, xlab="X", ylab="u or v", xlim=c(0,50), ylim=c(0,10)) 
  lines(X_all,u_1[2,], col=2)
  legend("topright", inset=0.02, 
       legend = c("u", "v"), title = paste0("t =",i*t.total_iter),
       col=c(1,2), lty = 1, cex=0.8)
