...
which is the force derived from the Lennard-Jones (LJ) potential [@lennardjones1924], the classic model of the interaction between neutral atoms. The force is repulsive ($F > 0$) for $r < 1$ and attractive ($F < 0$) for $r > 1$, crossing zero at the equilibrium separation $r = 1$. Its potential energy is $E(r) = \tfrac{1}{12} r^{-12} - \tfrac{1}{6} r^{-6}$, with a minimum at $r = 1$.
...
As in [Part 5B](05b-orbital-motion.qmd), the run is much easier to read as an animation than as a pair of snapshots. The code below renders the box simulation to a GIF; as before it is shown to run locally rather than executed at build time.
