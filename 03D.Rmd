...
for(i in seq_len(nrow(X_init_all))){
  results = RK4_2D(derivs_ts_orig, X_init_all[i,], t.total, dt)
  colnames(results) = c("t", "X", "Y")
  p1 = p1 + geom_path(data=as.data.frame(results), aes(x=X, y=Y), color = "blue")
}
X_init_all = array(runif(20, -1, 1), dim = c(10,2)) # generate 10 random initial conditions

for(i in seq_len(nrow(X_init_all))){
  X_init = X_init_all[i,] + c(189.96888, 126.64398)
  results = RK4_2D(derivs_ts_revised, X_init, t.total, dt)
