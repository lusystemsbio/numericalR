...
**Objective.** Plot the time course of a constitutively expressed gene from several
initial conditions and watch them settle to a common steady state.

**Method.** The exact solution of the rate equation `dX/dt = g - k*X`, which is
`X(t) = g/k + (X0 - g/k)*exp(-k*t)`.

**Test.** g = 50 (nM/min), k = 0.1 (1/min), initial values X0 = 300, 400, 500, 600,
700 nM, over t = 0 to 80 min.

**Verification.** Every curve begins at its own X0 and approaches the same steady
state `g/k = 500` nM; curves that start above 500 decay to it and those below rise
to it.
:::
...
methods for integrating ODEs in [Part 2B](02b-numerical-integration.qmd).
...
::: {.callout-note title="Recipe card"}
**Objective.** Plot the excitatory Hill function for several Hill coefficients and
see how cooperativity sharpens the response.

**Method.** The excitatory Hill function `H(X) = (X/Xth)^n / (1 + (X/Xth)^n)`,
evaluated over a range of X, where Xth is a threshold and n the Hill coefficient.

**Test.** Xth = 1, Hill coefficients n = 1, 2, 3, 4, 5, for X/Xth from 0 to 3.

**Verification.** Every curve rises from 0 at X = 0 toward 1 and passes through 0.5
at X = Xth; a larger n gives a steeper, more switch-like transition.
:::
...
nonlinear Hill term; we return to its numerical solution in [Part 2D](02d-effective-potential.qmd).
...
::: {.callout-note title="Recipe card"}
**Objective.** Plot the inhibitory Hill function for several Hill coefficients.

**Method.** The inhibitory Hill function `H(X) = 1 / (1 + (X/Xth)^n)`, evaluated over
a range of X, where Xth is a threshold and n the Hill coefficient.

**Test.** Xth = 1, Hill coefficients n = 1, 2, 3, 4, 5, for X/Xth from 0 to 3.

**Verification.** Every curve falls from 1 at X = 0 toward 0 and passes through 0.5
at X = Xth; it equals 1 minus the excitatory Hill function at every point.
:::
...
analyze the dynamics; we take this up in
[Part 3A](../03-phase-plane/03a-nullclines.qmd).
