...
**Objective.** Plot the time course $X(t)$ of a constitutively expressed gene from
several initial conditions and watch them settle to a common steady state.

**Method.** Evaluate the closed-form solution of $dX/dt = g - kX$, namely
$X(t) = g/k + (X_0 - g/k)\,e^{-kt}$ (Equation 6), over time.

**Test.** $g = 50$ nM/min, $k = 0.1$ min$^{-1}$, initial values
$X_0 = 300, 400, 500, 600, 700$ nM, for $t = 0$ to $80$ min.

**Verification.** Every curve should approach the same steady state
$X_s = g/k = 500$ nM; each begins at its own $X_0$ at $t = 0$; curves above 500 decay
to it and those below rise to it.
:::
...
**Method.** Evaluate $H^{ex}(X) = (X/X_{th})^{n} / (1 + (X/X_{th})^{n})$ (Equation 8)
over a range of $X$.

**Test.** $X_{th} = 1$, Hill coefficients $n = 1$ to $5$, for $X/X_{th} = 0$ to $3$.

**Verification.** Every curve rises from $0$ at $X = 0$ toward $1$ and passes through
$0.5$ at $X = X_{th}$; a larger $n$ gives a steeper, more switch-like transition.
:::
...
**Method.** Evaluate $H^{inh}(X) = 1 / (1 + (X/X_{th})^{n})$ (Equation 11) over a
range of $X$.

**Test.** $X_{th} = 1$, Hill coefficients $n = 1$ to $5$, for $X/X_{th} = 0$ to $3$.

**Verification.** Every curve falls from $1$ at $X = 0$ toward $0$, passes through
$0.5$ at $X = X_{th}$, and equals $1 - H^{ex}(X)$ at every point.
:::
