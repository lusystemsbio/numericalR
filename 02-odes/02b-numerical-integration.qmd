...
is a visible gap near t = 10 (error a few nM); both reach the steady state g/k = 500.
:::
...
::: {.callout-note title="Recipe card"}
**Objective.** Run the same Euler integration three ways, a hand-written loop, a
general ODE library, and a compiled routine, and compare their speed.

**Method.** The first-order Euler scheme for `dX/dt = g - k*X`, carried out by hand,
through an ODE library, and as compiled code called from R or Python, then timed.

**Test.** g = 50, k = 0.1, X0 = 300, from t = 0 to 80; benchmark at dt = 0.01.

**Verification.** All three produce the same trajectory; the timing shows the compiled
routine fastest, with the library no faster than the hand-written loop for this simple
fixed-step problem.
:::
...
its error by roughly a factor of 4 (second-order accuracy).
:::
...
**Method.** RK4: combine four slope evaluations across the step (at the start, twice at
the midpoint, and at the end) with weights 1, 2, 2, 1.

**Test.** Integrate `dX/dt = g - k*X` with g = 50, k = 0.1, X0 = 300.

**Verification.** Far more accurate than RK2 or Euler at the same step size, checked
against `X(t) = g/k + (X0 - g/k)*exp(-k*t)`; halving the step should cut its error by
roughly a factor of 16 (fourth-order accuracy).
:::
...
**Method.** The root-mean-square deviation between each integrator's result and the
exact solution, computed across a range of step sizes.

**Test.** `dX/dt = g - k*X` with g = 50, k = 0.1, X0 = 300, from t = 0 to 80, step
sizes from 0.1 to 1.

**Verification.** On a log scale the error falls with step size at a rate set by each
method's order: Euler shallowest (first order), Heun and RK2 steeper (second order),
RK4 steepest (fourth order). The exact solution is `X(t) = g/k + (X0 - g/k)*exp(-k*t)`.
:::
...
(where forward Euler diverges).

**Verification.** Forward Euler at dt = 0.2 oscillates and diverges, while backward
Euler stays stable and tracks the exact solution `X(t) = g/k + (X0 - g/k)*exp(-k*t)`
toward the steady state g/k = 5.
:::
