...
**Objective.** Integrate the gene-expression ODE $dX/dt = g - kX$ from an initial
condition and compare the numerical solution to the exact one at two step sizes.

**Method.** The Euler method, $X(t+\Delta t) = X(t) + f(X,t)\,\Delta t$ (Equation 2),
iterated from $t = 0$.

**Test.** $g = 50$, $k = 0.1$, $X_0 = 300$, over $t = 0$ to $80$, with $\Delta t = 1$
and $\Delta t = 0.1$.

**Verification.** Overlay the analytical solution from Part 2A: the $\Delta t = 0.1$
curve should track it closely, while $\Delta t = 1$ shows a visible gap near $t \approx 10$.
:::
...
::: {.callout-note title="Recipe card"}
**Objective.** Run the same Euler integration three ways, a hand-written loop, an
ODE library, and a compiled routine, and compare their speed.

**Method.** The first-order Euler scheme of 2B.1, carried out by hand, through an
ODE library, and as compiled code called from R or Python, then timed.

**Test.** The same system ($g = 50$, $k = 0.1$, $X_0 = 300$, $t = 0$ to $80$),
benchmarked at $\Delta t = 0.01$.

**Verification.** All three should produce the same trajectory; the timing should
show the compiled routine fastest, with the library no faster than the hand-written
loop for this simple fixed-step problem.
:::
...
::: {.callout-note title="Recipe card"}
**Objective.** Implement a second-order Heun integrator as a reusable function.

**Method.** Heun's method (Equation 4): take an Euler step, evaluate the slope at
that predicted point, and advance by the average of the two slopes.

**Test.** Integrate $dX/dt = g - kX$ ($g = 50$, $k = 0.1$, $X_0 = 300$) as in 2B.1.

**Verification.** At the same $\Delta t$ it should sit closer to the analytical
solution than Euler; its per-step error scales as $O(\Delta t^3)$ (measured in 2B.7).
:::
...
**Method.** RK2 (Equation 5): take a trial half-step to the midpoint of the interval
and use the slope there for the full step.

**Test.** The same system as 2B.1 ($g = 50$, $k = 0.1$, $X_0 = 300$).

**Verification.** Accuracy comparable to Heun (both second order), and better than
Euler at the same $\Delta t$ (measured in 2B.7).
:::
...
**Method.** RK4 (Equation 6): combine four slope evaluations across the step with
weights 1, 2, 2, 1.

**Test.** The same system as 2B.1 ($g = 50$, $k = 0.1$, $X_0 = 300$).

**Verification.** Far more accurate than RK2 or Euler at the same $\Delta t$; its
per-step error scales as $O(\Delta t^5)$ (measured in 2B.7).
:::
...
::: {.callout-note title="Recipe card"}
**Objective.** Measure how each integrator's error shrinks as the step size decreases.

**Method.** Compute the root-mean-square deviation (RMSD) between each integrator's
result and the exact solution across a range of $\Delta t$.

**Test.** $dX/dt = g - kX$ ($g = 50$, $k = 0.1$, $X_0 = 300$, $t = 0$ to $80$),
with $\Delta t$ from $0.1$ to $1$.

**Verification.** On a log scale the error falls with $\Delta t$ at a rate set by each
method's order (Euler shallowest, RK4 steepest); a compiled RK4 matches the
hand-written RK4 to a tiny RMSD.
:::
...
::: {.callout-note title="Recipe card"}
**Objective.** Integrate a stiff version of the model where forward Euler blows up,
using an implicit step.

**Method.** Backward (implicit) Euler (Equation 8), evaluating the derivative at the
new point; for this linear $f$ it has the closed form $X_{n+1} = (X_n + hg)/(1 + hk)$.

**Test.** Stiff parameters $g = 50$, $k = 10$, $X_0 = 3$, $t = 0$ to $4$,
$\Delta t = 0.2$ (where forward Euler diverges).

**Verification.** Forward Euler at $\Delta t = 0.2$ oscillates and diverges, while
backward Euler stays stable and tracks the exact solution.
:::
