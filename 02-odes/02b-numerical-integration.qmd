...
subtitle: "Part 2: Ordinary differential equations — B"
author: "Mingyang Lu"
old_id: "02B"
---

Here we introduce numerical methods to solve ODEs, using the constitutively
expressed gene circuit from the previous section as a running example:
...
![](images/02b/fig1.png){width=400 fig-align="center"}
...
$\Delta t \to 0$ — it bounds the error of the expansion. Comparing Equations (1)
...
Both the hand-written loop and the compiled Fortran routine are typically faster
than the general library solver for this simple, fixed-step problem: the library
carries per-step overhead (adaptive error control, list/dictionary bookkeeping),
while the compiled routine avoids interpreter overhead entirely. Exact timings
vary by machine and run. The lesson is not that libraries are slow — they are
robust and general — but that for tight inner loops, a compiled kernel can be an
...
In the Euler method the derivative is treated as constant over
$t \to t+\Delta t$, which works when $\Delta t$ is small and the derivative
varies slowly. Higher-order methods reduce the error. Heun's method
[@hairer1993solving] is a second-order scheme:

![](images/02b/fig2.png){width=600 fig-align="center"}

Starting from point A $(X(t), t)$, an Euler step reaches point B; the derivative
at B is then averaged with the derivative at A to produce a better estimate:
...
A Taylor-expansion argument (using
$\frac{df}{dt} = \frac{\partial f}{\partial t} + f\frac{\partial f}{\partial X}$)
shows the local error is $O(\Delta t^3)$.
