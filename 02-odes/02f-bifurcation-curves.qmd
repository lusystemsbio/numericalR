...
point on the curve and walks along it with a predictor–corrector step; its
**arc-length** variant parameterizes the curve by distance along itself, so it
rounds the turning points at the saddle-node bifurcations, where $X$ is
multivalued in $k$ and both the sampling methods and naive continuation struggle.
These are the general-purpose tools of bifurcation analysis, and they return the
curve already ordered.
...
`derivs_k` used in Part 2E rather than relying on it.
