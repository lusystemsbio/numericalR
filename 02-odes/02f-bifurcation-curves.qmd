...
**Method.** Treat `z = f(k, X)` as a surface over the (k, X) plane; the steady states
are its zero-level contour, extracted by a standard contour algorithm on a grid.

**Test.** The self-activating gene `f(X, k)` evaluated over a grid of (k, X).

**Verification.** The zero contour is the same S-shaped curve as in Part 2E, but
returned as connected line segments rather than a scattered point cloud; one contour
captures the whole connected curve.
:::
...
**Method.** Numerical continuation: from a known point on `f(X, k) = 0`, predict along
the tangent `dX/dk = -(df/dk) / (df/dX)`, then correct back onto the curve with a root
solve at the new k.

**Test.** The self-activating gene, continued across a range of k.

**Verification.** It follows a branch accurately away from folds but stalls at a fold
(turning point), where `dX/dk` diverges because k is multivalued there.
:::
...
**Method.** Arc-length continuation: parameterize the curve by arc length s instead of
k, splitting each step into `dk` and `dX` with `ds^2 = dk^2 + dX^2` and `dX = h*dk`
(where `h = dX/dk`), so the step stays finite where `dX/dk` diverges.

**Test.** The self-activating gene across the full bistable range.

**Verification.** The curve is traced smoothly through both folds, recovering the
complete S-shape (both stable branches and the unstable middle) where plain
k-continuation (2F.5) stalled.
:::
