...
To distribute a vaccine efficiently, we want the shortest route that visits each of $N = 50$ cities once and returns to the distribution center (the first city). The file `us50.txt` gives the city coordinates, which we load and plot below.

### R implementation {.impl}

```{r}
us50 <- read.table("09-optimization/data/us50.txt", col.names = c("x", "y"))
...
us50 = np.loadtxt("09-optimization/data/us50.txt")   # x, y columns
# open circles and no forced aspect ratio, matching R's default plot style
plt.scatter(us50[:, 0], us50[:, 1], facecolors="none", edgecolors="black")
plt.xlabel("x"); plt.ylabel("y")
plt.show()
```

An example (non-optimal) route through these cities is shown below.
...
#| fig-height: 3.2
#| out-width: "95%"
df <- read.csv("09-optimization/data/timeseries.csv")
...
df = pd.read_csv("09-optimization/data/timeseries.csv")
plt.plot(df["time"], df["value"], color="black", linewidth=2)   # black, as in R
