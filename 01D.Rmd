...
$\Delta \mathbf{r_j}$ is defined as

$$\Delta \mathbf{r_j} = \mathbf{r_j} - \mathbf{\overline{r}}$$

Hint: Pay attention the dot product operation ($\cdot$) in the definition of $R_g$. Try the following for the usage of atom2mass
...
Part B shows how to benchmark code performance with microbenchmark. Here, we will benchmark more using the same approach. We would like to compute $\ln n!$. For small $n$, this can be done with the following code:
...
However, for large $n$ (*e.g.*, 1000), this method fails (think why?). In this case, we can express $\ln n!$ as the following. 

$$f(n) = \ln n!=\sum_{j=1}^{n}\ln j$$

**2.1** Create R functions to compute $f$ as the function of $n$ using the following methods: 
...
**2.2** In mathematics, there is famous formula called Stirling's approximation.
