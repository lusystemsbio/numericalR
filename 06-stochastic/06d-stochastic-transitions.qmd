...
axes to do so), giving a handful of transitions each way over this run. The count
is only a rough estimate, though: it can treat a brief dip across the boundary as a
transition, so it tends to over-count incomplete excursions, and because R and
Python draw different random streams they report somewhat different numbers. That
run-to-run sensitivity, together with the slowness of the point-by-point R loop, is
why we turn to a more robust statistic.
