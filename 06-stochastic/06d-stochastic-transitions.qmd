...
repressing two-gene circuit whose deterministic phase plane we studied in Part 3.
...
# state sequence over the trajectory, then count crossings
state_all <- unlist(apply(results[, 2:3], 1, function(X) get_state(X, ell1, ell2)))
n_12 <- sum(diff(state_all) ==  1)    # transitions state 1 -> 2
n_21 <- sum(diff(state_all) == -1)    # transitions state 2 -> 1
c(state1_to_2 = n_12, state2_to_1 = n_21)
```
...
d = np.diff(states)
print("state1_to_2:", int(np.sum(d == 1)), " state2_to_1:", int(np.sum(d == -1)))
```

The ellipses capture most of each state's cloud (the factor of 1.5 enlarges the
axes to do so). The crossing count, however, is unreliable: it treats every brief
dip across the boundary as a transition, so it badly over-counts incomplete
excursions that immediately fall back, and the result depends heavily on the
particular noise realization. The R and Python runs here draw different random
streams and, tellingly, report very different counts, one modest and one many
times larger. That sensitivity, together with the slowness of the point-by-point
R loop, is why we turn to a more robust statistic.
