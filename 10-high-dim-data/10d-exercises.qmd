...
The following builds and draws the graph. The edge list has a regular structure that
is worth exposing in the drawing: vertices 1 to 5 form one chain, 6 to 10 form another,
and each vertex $i$ in the first chain has an edge to $i + 5$ in the second. Placing the
two chains as two rows, one directly above the other, turns those edges into vertical
rungs and leaves the remaining "skip" edges (1 to 3, 10 to 6, and so on) as arcs, which
is far easier to read than an automatic layout.
...
# two-row layout: vertices 1-5 on top, 6-10 directly below
v <- as.integer(V(graph)$name)
lay <- cbind(ifelse(v <= 5, v - 1, v - 6), ifelse(v <= 5, 1, 0))
# curving the edges lifts each skip edge off the vertices it would otherwise pass
# through; rescale = FALSE keeps the wide two-row shape, which igraph would
# otherwise stretch to a square and so lengthen the rungs
plot(graph, layout = lay, rescale = FALSE, asp = 0,
     xlim = c(-0.25, 4.25), ylim = c(-0.55, 1.55),
     edge.curved = 0.3, edge.label = edges$weight, edge.arrow.size = 0.5,
     vertex.size = 22, vertex.color = "gold", vertex.frame.color = NA,
     edge.color = "#B3B3B3")
```
...
# the same two-row layout, built directly as coordinates
pos = {i: (i - 1, 1) if i <= 5 else (i - 6, 0) for i in graph.nodes}
# matplotlib's rad uses the opposite sign convention to igraph's edge.curved, so a
# negative rad here bows the arcs outward exactly as edge.curved = 0.3 does in R
curve = "arc3,rad=-0.25"
...
nx.draw(graph, pos, with_labels=True, node_color="gold", edge_color="#B3B3B3",
        font_color="darkblue", node_size=600, connectionstyle=curve)
...
The corresponding network is shown below (pink: the three signalling nodes, which serve as the input control; gold: all other nodes, both signalling effectors and transcription factors; green edges: activation; red edges: inhibition).
