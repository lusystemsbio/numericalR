...
This chapter applies the phase-plane toolkit of [Part 3A](03a-nullclines.qmd) and [Part 3B](03b-steady-states-2d.qmd) to a different two-variable system, the classic **chemostat** model of bacterial growth in continuous culture [@sontag_systemsbio]. It introduces no new methods; the numerical steps are the same as before (RK4 integration, vector fields, nullclines, steady-state stability), so the focus is on setting up a new model and reusing that machinery in both R and Python. Practice questions are embedded throughout.
...
Implement arc-length numerical continuation for the C-nullcline, as in [Part 3A](03a-nullclines.qmd) and [Part 2F](../02-odes/02f-bifurcation-curves.qmd). Parameterize the curve by arc length $s$ so the tracing survives points where $\frac{dC}{dN} \to \infty$, with a Newton correction to stay on $g(N, C) = 0$.
:::

## 3C.4 Steady states and stability

The steady states are the intersections of the two nullclines, and their stability follows from the eigenvalues of the Jacobian. Both the line-intersection routines (`find_intersection`, `find_intersection_all_fast`) and the numerical Jacobian classifier (`stability_2D`) are exactly the ones built in [Part 3B](03b-steady-states-2d.qmd), so rather than repeat them we apply them directly to the chemostat's nullclines.
...
The N-nullcline is stored as two separate line pieces ($N = 0$ and $C = \frac{1}{\alpha_1 - 1}$), so running the all-pairs intersection search of [Part 3B](03b-steady-states-2d.qmd) can report the same steady state more than once. Why does that happen, and how would you deduplicate the results? Then apply `find_intersection_all_fast` and `stability_2D` to reproduce the classification above.
:::
