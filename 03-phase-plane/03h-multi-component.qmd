...
**Method.** Integrate the nondimensionalized system `dx/dt = g/(1 + y^3) - x`, `dy/dt = h*x^3/(1 + x^3) - y` with the generic RK4.

**Test.** The negative-feedback circuit at the chapter's g and h.
...
**Method.** Integrate `dx/dt = g/(1 + y^3) - x`, `dy/dt = h/(1 + x^3) - y` with the generic RK4.

**Test.** The double-negative circuit at the chapter's g and h, from several initial conditions.
...
**Method.** Integrate `dx/dt = g/(1 + z^3) - x`, `dy/dt = h/(1 + x^3) - y`, `dz/dt = l/(1 + y^3) - z` with the generic RK4.

**Test.** The three-gene ring at the chapter's parameters.
...
**Method.** A generalized Lotka-Volterra model of S species, `dN_i/dt = N_i*(1 - sum_j a_ij*N_j) + D`, with self-terms `a_ii = 1`, random off-diagonals, and a small dispersal D, integrated with the generic RK4.

**Test.** S interacting bacterial species with a random interaction matrix.
