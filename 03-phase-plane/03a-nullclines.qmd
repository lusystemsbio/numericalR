...
**Objective.** Simulate a two-gene toggle switch from many initial conditions and see which steady states they settle on.

**Method.** A vector form of the RK4 integrator, advancing the state (X, Y) with the same four-stage scheme used for one variable.

**Test.** The toggle switch (X and Y repress each other), started from ten random initial conditions.
...
**Test.** The toggle-switch nullclines.
...
**Test.** The toggle switch on a grid over the (X, Y) plane.
...
**Test.** The toggle switch's X-nullcline `fX(X,Y) = 0`.
