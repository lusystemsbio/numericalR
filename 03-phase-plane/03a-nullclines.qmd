...
Part 3 generalizes the single-variable methods of [Part 2](../02-odes/02a-modeling-gene-circuits.qmd) to systems of *two* coupled ODEs, using a two-gene circuit as the running example. The main new tool is **phase-plane analysis** [@strogatz2015nonlinear], a visual way to characterize a nonlinear system by plotting its state in the plane of its two variables.

We model the genetic **toggle switch** from [Part 2A](../02-odes/02a-modeling-gene-circuits.qmd) (Equation (13)): two genes $X$ and $Y$ that each repress the other [@gardner2000toggle], with dynamics
...
To integrate a multi-variable system we generalize the fourth-order Runge-Kutta method of [Part 2B](../02-odes/02b-numerical-integration.qmd) so that the state $X$ and each stage $k_i$ are vectors rather than scalars; the algorithm is otherwise identical. Simulating from ten random initial conditions, every trajectory settles on one of **two** distinct stable steady states (different levels of $X$ and $Y$), the hallmark of the toggle switch's bistability.
...
When separation of variables is not possible, the nullcline $f_X(X, Y) = 0$ is just the zero-level contour of the surface $Z = f_X(X, Y)$, exactly as in [Part 2F](../02-odes/02f-bifurcation-curves.qmd). Evaluating $f_X$ and $f_Y$ on a grid and drawing their $Z = 0$ contours recovers both nullclines. The method is robust as long as the grid is fine enough for the contour routine to interpolate, though a fine grid over two dimensions can be costly.
...
The third option is **numerical continuation**, the arc-length predictor-corrector of [Part 2F](../02-odes/02f-bifurcation-curves.qmd). Tracing the nullcline $f_X(X, Y) = 0$ is the same problem as tracing a bifurcation curve: we treat $Y$ as the control parameter and follow $X(Y)$, stepping along the curve by arc length and correcting each step back onto it with Newton's method. The corrector needs the partial derivatives $\partial f/\partial X$ and $\partial f/\partial Y$ (the Jacobian). The code below adapts the 2F routine to two variables; because the method is identical to 2F, the Python implementation is omitted (see [Part 2F](../02-odes/02f-bifurcation-curves.qmd)).
...
The continuation reproduces the same two nullclines as separation of variables and the contour method. It is more code, since it needs a starting point on each curve, the travel direction, and the Jacobian, and (as written) it has no guard against a diverging Newton step. The advantage, as in Part 2F, is that it applies to nullclines that cannot be solved analytically. All three methods feed the same next step: [Part 3B](03b-steady-states-2d.qmd) locates the nullcline intersections to find the steady states and classify their stability.
