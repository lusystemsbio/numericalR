...
In [Part 5C](../05-molecular-dynamics/05c-box-of-particles.qmd) we studied a box of particles interacting through the Lennard-Jones potential by *molecular dynamics*, integrating Newton's equations to follow the trajectory in time. Here we study the same box with the *Metropolis-Hastings algorithm* of [Part 8B](08b-metropolis-algorithm.qmd), sampling configurations directly from the Boltzmann distribution instead of propagating dynamics.
...
with a minimum at $r = 1$. We write the potential as a function of $r^2$ rather than $r$, which lets the pairwise routines skip a square root. Distances use the periodic box and the minimum image convention introduced in [Part 5C](../05-molecular-dynamics/05c-box-of-particles.qmd), so each particle interacts with the nearest image of every other particle.
