...
The stochastic differential equations of [Part 6](../06-stochastic/06a-random-number-generators.qmd) add continuous noise to a deterministic rate equation. For a chemical reaction network with small molecule numbers, a different and more fundamental description is often needed: the molecules are discrete and react one event at a time. The *Gillespie algorithm* [@gillespie1976general; @gillespie1977exact], also called the stochastic simulation algorithm (SSA), generates exact sample trajectories of such a network from its chemical master equation.
...
2. draw the waiting time $\tau$ to the next event from an exponential distribution with rate $R_\text{tot} = \sum_j R_j$;
3. pick which reaction fires, choosing reaction $j$ with probability $R_j / R_\text{tot}$;
4. advance time $t \rightarrow t + \tau$ and update the state $\mathbf{S} \rightarrow \mathbf{S} + \boldsymbol{\gamma}_j$.
