...
```{r}
ode1 <- function(X0, t.total, g, k) {   # analytical solutions, see Part 2A
  return(X0 + (g/k - X0)*(1-exp(-k*t.total)))
}
t_all = seq(0, 80, 0.1)    # time points to plot

dX <- function(t, X, g, k) { # define derivative function, Equation (1)
  return(g - k*X)
}

# Euler is a generic function of Euler method for a single ODE 
# Note that ellipsis (...) is used ot pass any argument to the function. 
...
plot(t_all, ode1(300, t_all, g, k), type = "l", col=2,
...
# params take the g and k in a vector of size 2
...
for(dt in dt_all){
  ...
  results_ode = ode1(300,t_all, g, k) # ode1 is defined above -- exact solutions
  results_Heun = Heun(dX, 300, 80, dt, g, k) # dX is defined above,  dX/dt = g - k * X
  results_RK2 = RK2(dX, 300, 80, dt, g, k) 
  results_RK4 = RK4(dX, 300, 80, dt, g, k) 
  results_Euler = Euler(dX, 300, 80, dt, g, k)   #Euler is defined above
...
results7 = RK4(dX, 300, 80, 0.1, g, k) 
results8 = RK2(dX, 300, 80, 0.1, g, k) 
results9 = Euler(dX, 300, 80, 0.1, g, k)
results10 = ode1(300,seq(0,t_total,dt), g, k) # exact solutions
