...
subtitle: "B2. Practice: modeling chemostat"
author: "Mingyang Lu"
date: "02/13/2022"
output:
  html_document:
...
In this case, we will use RK4 integrator to simulate the ODEs defined by Equation (1). 

```{r}
RK4_2params <- function(derivs, X0, t.total, dt, alpha1, alpha2){ # Runge-Kutta 4th order, allowing two parameters in the derivs function
  ...
  for (i in 1:(n_all-1)) {
    ...
    t_0.5 = t_0 + 0.5*dt
    t_1 = t_0 + dt
    k1 = dt * derivs(t_0,X_all[i,], alpha1, alpha2)
    k2 = dt * derivs(t_0.5,X_all[i,] + k1/2, alpha1, alpha2)
    k3 = dt * derivs(t_0.5,X_all[i,] + k2/2, alpha1, alpha2)
    k4 = dt * derivs(t_1,X_all[i,] + k3, alpha1, alpha2)
...
results_chemostat_1 = RK4_2params(derivs = derivs_chemostat, X0 = X0_1, t.total = 20, dt = 0.01, alpha1, alpha2)
...
results_chemostat_2 = RK4_2params(derivs = derivs_chemostat, X0 = X0_2, t.total = 20, dt = 0.01, alpha1, alpha2)
...
p1 +
   geom_line(data = as.data.frame(results_chemostat_2), aes(x=N, y=C, colour = "Traj 2"), size=1) 
```
...
for(i in seq_len(nrow(X_init_all))){
  results = RK4_2params(derivs_chemostat, X_init_all[i,], t.total, dt, alpha1, alpha2)
...
p1 +
  geom_line(data = as.data.frame(null1_1), aes(x=N, y=C, colour = "dN/dt=0"), size=1) + 
  geom_line(data = as.data.frame(null1_2), aes(x=N, y=C, colour = "dN/dt=0"), size=1) + 
  geom_line(data = as.data.frame(null2), aes(x=N, y=C, colour = "dC/dt=0"), size=1) 
```
...
p1 +
  geom_line(data = as.data.frame(null1_1), aes(x=N, y=C, colour = "dN/dt=0"), size=1) + 
  geom_line(data = as.data.frame(null1_2), aes(x=N, y=C, colour = "dN/dt=0"), size=1) + 
  geom_line(data = as.data.frame(null2_keep), aes(x=N, y=C, colour = "dC/dt=0"), size=1) 
...
p1 +
  geom_line(data = as.data.frame(null2_ver2), aes(x=N, y=C, colour = "dC/dt=0"), size=1) 
...
The expression is defined as a function h, which will be used later. Therefore, starting from a steady state ($N$, $C$), we can obtain the slop of the nullcline $\frac{dC}{dN}$ from equation (3). This allows us to find the initial guess of the next ($N$, $C$) point. 
...
<center> ![arc length continuation](./extra/data/02D/fig1.png){width=50%} </center>
<br/>
...
``` {r}
func_f <- function(N, C, alpha1, alpha2) {       # dN/dt
  return(alpha1 * N*C/(1+C) - N)
}
func_g <- function(N, C, alpha1, alpha2) {       # dC/dt
...
dNdt = func_f(null2_keep[,1], null2_keep[,2],alpha1, alpha2)  #dN/dt along the nullcline dC/dt = 0
dCdt1 = func_g(null1_1[,1], null1_1[,2],alpha1, alpha2)  #dY/dt along the nullcline dX/dt = 0 first part
dCdt2 = func_g(null1_2[,1], null1_2[,2],alpha1, alpha2)  #dY/dt along the nullcline dX/dt = 0 second part

ss_all1 = find_intersection_all_fast(null1_1, null2_keep, dNdt, dCdt1)
ss_all1

ss_all2 = find_intersection_all_fast(null1_2, null2_keep, dNdt, dCdt2)
ss_all2
...
stability <- function(func_fx, func_fy, ss, ...) { # ss is a vector of steady state values X_S, Y_S
  ...
  dfxdx = (func_fx(ss[1]+delta, ss[2], ...) - func_fx_current)/delta
  dfxdy = (func_fx(ss[1], ss[2]+delta, ...) - func_fx_current)/delta
  dfydx = (func_fy(ss[1]+delta, ss[2], ...) - func_fy_current)/delta
  dfydy = (func_fy(ss[1], ss[2]+delta, ...) - func_fy_current)/delta
  ...
  print("Jacobian:")
  print(jacobian)
  print(c("eigenvalues:",lambda))
  return(stability)
}

ss_with_stability =  t(apply(ss_all, MARGIN = 1, function(ss) {
                          return(c(ss,stability(func_f, func_g, ss,alpha1,alpha2)))}))
